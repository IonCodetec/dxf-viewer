...
import { pickEntityAt } from "./PickingUtils.js";
...
export class DxfViewer {
  ...
  constructor(domContainer, options = null) {
    ...
    this.canvas.addEventListener("pointerdown", this._OnCanvasPick.bind(this));
  ...
  Render() {
    ...
    // --- INICIO BLOQUE DE RESALTADO ---
    if (this.dxfScene && this.dxfScene.selectedEntityId != null) {
      for (const layer of this.layers.values()) {
        for (const obj of layer.objects) {
          if (
            obj.userData &&
            obj.userData.entityId === this.dxfScene.selectedEntityId
          ) {
            obj.material.color.set(0xff0000); // Color de resaltado (rojo)
            obj.material.needsUpdate = true;
          } else if (obj.userData && obj.userData.originalColor) {
            obj.material.color.set(obj.userData.originalColor); // Restaurar color original
            obj.material.needsUpdate = true;
          }
        }
      }
    }
    // --- FIN BLOQUE DE RESALTADO ---
  ...
  _OnCanvasPick(e) {
    // Solo picking con botón izquierdo
    ...
    const pos = this._CanvasToSceneCoord(canvasCoord.x, canvasCoord.y);

    if (this.dxfScene && this.dxfScene.cpuEntities) {
      const seleccionada = pickEntityAt(
        this.dxfScene.cpuEntities,
        pos.x,
        pos.y
      );

      if (seleccionada) {
        this.selectedEntityId = seleccionada.id;
      } else {
        this.selectedEntityId = null;
      }

      // Haz accesible el ID seleccionado en la escena lógica para el render
      this.dxfScene.selectedEntityId = this.selectedEntityId;

      this.Render(); // Actualiza la visualización
      this._Emit("entitypicked", { entity: seleccionada });
    }
  }
...
class Batch {
  ...
  *_CreateObjects(instanceBatch) {
    ...
    function CreateObject(vertices, indices) {
      ...
      obj.userData.entityId = this.key.id;
      obj.userData.originalColor = material.color.getHex();
      return obj;
    }
