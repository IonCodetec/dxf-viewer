...
/**
 * Añade una entidad a cpuEntities según su tipo.
 * @param {Array} cpuEntities - El array donde se guardan las entidades seleccionables.
 * @param {Object} entity - La entidad procesada.
 */
function addEntityToCpuEntities(cpuEntities, entity) {
  // Usa un id único (puedes adaptar a tu lógica)
  const id = entity.handle || cpuEntities.length;

  switch (entity.type) {
    case "LINE":
    case 1: // Por si usas un enum numérico
      if (entity.vertices && entity.vertices.length >= 2) {
        cpuEntities.push({
          ...
          bounds: calcularBoundingBox(entity.vertices),
        });
      }
      break;
    ...
    case 4:
      if (
        ...
        // Calcula el bounding box del arco (simplificado: usa el círculo completo para empezar)
        cpuEntities.push({
          id,
          type: "ARC",
          center: { x: entity.center.x, y: entity.center.y },
    ...
    case 0:
      if (entity.vertices && entity.vertices.length) {
        cpuEntities.push({
          id,
          type: "POINT",
          position: { x: entity.vertices[0].x, y: entity.vertices[0].y },
          bounds: calcularBoundingBox(entity.vertices),
        });
      }
      break;
    ...
    default:
      // console.log('Tipo de entidad no soportado para selección:', entity.type);
      break;
  }
}
...
export class DxfScene {
  ...
  _ProcessDxfEntity(entity, blockCtx = null) {
    ...
    addEntityToCpuEntities(this.cpuEntities, entity);
  ...
  *_DecomposeText(entity, blockCtx) {
    ...
    const fixedHeight =
      style?.fixedTextHeight === 0 ? null : style?.fixedTextHeight;
    yield* this.textRenderer.Render({
      text: ParseSpecialChars(entity.text),
  ...
  *_DecomposeHatch(entity, blockCtx) {
    ...
    if (entity.isSolid) {
      const coords = this._TransformBoundaryLoop(
        filteredBoundaryLoops[0],
        transform
      );
      const holes = [];
      for (let i = 1; i < filteredBoundaryLoops.length; i++) {
        holes.push(coords.length / 2);
        this._TransformBoundaryLoop(
          filteredBoundaryLoops[i],
          transform,
          coords
        );
      }
